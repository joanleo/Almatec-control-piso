document.addEventListener('DOMContentLoaded', function(){
	...
	document.querySelector('[data-action="volver"]').addEventListener('click', function() {
	...
	function loadItemRoute(item) { // Modificar para recibir el objeto item completo
	    const routeContainer = document.getElementById('routeContainer');
	    const timelineContainer = routeContainer.querySelector('.timeline-container');
	    
	    // Ocultar resultados y mostrar contenedores
	    document.getElementById('searchResults').style.display = 'none';
	    routeContainer.style.display = 'block';
	    timelineContainer.style.display = 'block';
	    
	    // Actualizar detalles del item
	    document.getElementById('itemDescription').textContent = item.descripcion;
	    document.getElementById('itemCode').innerHTML = `<i class="fa fa-hashtag me-1"></i>GY${item.idItem}`;
	    document.getElementById('itemPlano').innerHTML = item.plano ? 
	        `<i class="fa fa-file-alt me-1"></i>${item.plano}` : '';
	    document.getElementById('itemTipo').className = `${getBadgeColor(item.tipo)} ms-2`;
	    document.getElementById('itemTipo').textContent = item.tipo;
	    ...
	    fetch(`/ingenieria/items/${item.idItem}/ruta`)
	        .then(response => response.json())
	        .then(routeData => {
	            renderRoute(routeData, timelineContainer);
	        })
	        .catch(error => {
	            console.error('Error:', error);
	            timelineContainer.innerHTML = `
	                <div class="alert alert-danger" role="alert">
	                    Error al cargar la ruta del item. Por favor, intenta nuevamente.
	                </div>
	            `;
	        });
	}

	function renderRoute(routeData, container) {
	    // Renderizar timeline
		console.log(routeData)
	    container.innerHTML = routeData.map((centro, index) => `
	        <div class="timeline-item">
	            <div class="timeline-number">${index + 1}</div>
	            <div class="timeline-card">
	                <h6 class="mb-2">${centro.centroTrabajo}</h6>
	                <div class="info-row mb-2">
	                    <span class="me-2"><i class="fa-solid fa-screwdriver-wrench"></i></span>
	                    <span>Id centro trabajo: ${centro.idCentroTrabajo}</span>
	                </div>
	                
					<div class="info-row mb-2">
	                    <span class=" me-2"><i class="fa-solid fa-clipboard-list"></i></span>
	                    <span>Valor aplicar reporte: ${centro.tiempoEstandar} ${centro.um}</span>
	                </div>
					<div class="info-row mb-2">
	                    <span class="me-2"><i class="fa-solid fa-screwdriver-wrench"></i></span>
	                    <span>Id centro trabajo ERP: ${centro.idCentroTrabajoErp}</span>
	                </div>
	                <div class="text-muted">
	                    <small>Código CT: ${centro.codigoCT}</small>
	                </div>
	            </div>
	        </div>
	    `).join('');
		...
		/*
		<div class="info-row mb-2">
            <span class="me-2"><i class="fa-solid fa-stopwatch"></i></span>
            <span>Tiempo estándar: ${centro.tiempoEstandar} hrs</span>
        </div>
		*/

	    // Actualizar resumen
	    //const totalTime = routeData.reduce((acc, curr) => acc + parseFloat(curr.tiempoEstandar), 0);
	    //document.getElementById('totalTime').textContent = totalTime.toFixed(1) + ' hrs';
	    document.getElementById('totalCenters').textContent = routeData.length;
	}
	
	function createItemElement(item) {
        const div = document.createElement('div');
        div.className = 'list-group-item list-group-item-action search-result';
        div.onclick = () => loadItemRoute(item);
